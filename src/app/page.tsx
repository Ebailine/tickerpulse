...
type Item = {
  ...
  summary?: string; ticker?: string;
  sentiment?: "bullish"|"bearish"|"neutral"
};
type PriceResp = {
  ticker: string;
  series: { d: string; close: number }[];
...
export default function Home() {
  ...
  async function refreshNews() {
    ...
    setNews(n.items); setSec(s.items);
  }

  async function refreshPrices() {
    const entries = await Promise.all(watchlist.slice(0,8).map(async t => {
      const r = await fetch(`/api/prices?t=${encodeURIComponent(t)}`).then(r=>r.json());
      return [t, r] as const;
    }));
    const obj: Record<string, PriceResp> = {};
    entries.forEach(([t, r]) => obj[t] = r);
    setPrices(obj);
  }
  ...
  const filteredNews = useMemo(() => {
    ...
    return news.filter(i => (i.title+" "+(i.summary||"")+(i.ticker||"")).toLowerCase().includes(needle));
  }, [q, news]);
        ...
        {!mounted ? (
          ...
          <>
            <div className="flex items-center justify-between">
              ...
              <button onClick={refreshAll} className="rounded-xl bg-slate-900 px-4 py-2 text-white disabled:opacity-50" disabled={loading}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {watchlist.map(t => (
                <span key={t} className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-sm">
                  {t}
                  <button onClick={()=>setWatchlist(watchlist.filter(x=>x!==t))} className="text-slate-500 hover:text-slate-900">×</button>
                </span>
              ))}
              <TickerAdder onAdd={(t)=> setWatchlist(Array.from(new Set([...watchlist, t.toUpperCase()])))} />
            </div>
    ...
    <div className="mt-4 grid gap-6 md:grid-cols-3">
      <div className="md:col-span-2">
        <div className="mb-3 flex items-center justify-between">
          ...
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search headlines or tickers" className="rounded-lg border px-3 py-2 text-sm" />
        </div>
        <ul className="space-y-3">
          {news.map((it, idx) => (
            <li key={idx} className="rounded-2xl border border-white/20 bg-white/70 backdrop-blur p-4 shadow-soft dark:bg-brand-900/60 dark:border-white/10 shadow-sm">
              ...
              {it.summary && <p className="mt-2 text-sm text-slate-700 line-clamp-3" dangerouslySetInnerHTML={{__html: it.summary}} />}
            </li>
          ))}
        </ul>
      </div>
          ...
          <ul className="mt-2 space-y-2 text-sm">
            {sec.slice(0,12).map((it, idx) => (
              <li key={idx} className="flex items-center justify-between gap-2">
                <a href={it.link} target="_blank" className="hover:underline">{it.title}</a>
              </li>
            ))}
          </ul>
        </div>
...
function PricesPanel({ watchlist, prices, onReload }:{ watchlist:string[]; prices:Record<string, PriceResp>; onReload:()=>void }) {
  return (
    <div className="mt-4">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-xl font-semibold">Prices</h2>
        <button onClick={onReload} className="rounded-lg border px-3 py-1.5 text-sm hover:bg-slate-50">Reload prices</button>
      </div>
      <div className="grid gap-3 md:grid-cols-2">
        {watchlist.map(t => <PriceCard key={t} data={prices[t]} ticker={t} />)}
      </div>
      {!watchlist.length && <div className="text-slate-600 text-sm mt-4">Add tickers above to load prices.</div>}
    </div>
  );
}
...
function Sparkline({ series }:{ series: { d:string; close:number }[] }) {
  ...
  const path = xs.map((i,idx)=> (idx? "L":"M") + px(i) + " " + py(ys[i])).join(" ");
  return (
...
function SignalsPanel({ watchlist, news, prices }:{ watchlist:string[]; news: Item[]; prices: Record<string, PriceResp> }) {
  const byTicker = (t:string)=> news.filter(n=>n.ticker===t);
  const avgSentiment = (items:Item[])=>{
    if (!items.length) return 0;
    const v = items.map(i => i.sentiment==="bullish"?1 : i.sentiment==="bearish"? -1 : 0).reduce((a,b)=>a+b,0);
  ...
  const rows = watchlist.map(t => {
    const s = avgSentiment(byTicker(t));
    const m = momentum(prices[t]);
    ...
    <div className="mt-4">
      <div className="rounded-2xl border border-white/20 bg-white/70 backdrop-blur p-4 shadow-soft dark:bg-brand-900/60 dark:border-white/10">
        ...
        <table className="mt-3 w-full text-sm">
          <thead><tr className="text-left"><th className="p-2">Ticker</th><th className="p-2">Avg sentiment</th><th className="p-2">20d momentum</th><th className="p-2">Action</th></tr></thead>
          <tbody>
            {rows.map(r=>(
              <tr key={r.t} className="border-t">
                ...
                <td className="p-2"><span className={cls("rounded-full px-2 py-1 text-xs",
